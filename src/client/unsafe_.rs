...
impl UnsafeClient {
    ...
    /// or receive.
    pub(crate) unsafe fn execute(&self, request: &str) -> Result<Option<&str>> {
        let crequest = CString::new(request).map_err(Error::NulError)?;
        ...
        Ok(Some(answer.to_str().map_err(Error::Utf8Error)?))
    }

    pub(crate) fn send(&self, request: &str) -> Result<()> {
        let crequest = CString::new(request).map_err(Error::NulError)?;
    ...
    /// UNSAFE: the returned slice is invalidated upon the next call to execute
    /// or receive.
    pub(crate) unsafe fn receive(&self, timeout: Duration) -> Result<Option<&str>> {
